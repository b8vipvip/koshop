    ...
    <div v-if="open" class="fixed inset-0 z-[100] flex items-end justify-center bg-black/30 lg:items-center lg:p-6">
      <section class="flex h-[100dvh] w-full flex-col overflow-hidden bg-[#f3f5f7] lg:h-[90vh] lg:max-w-[460px] lg:rounded-3xl">
        ...
        <footer class="border-t bg-white p-3 pb-[max(12px,env(safe-area-inset-bottom))]">
          ...
          <form class="flex items-center gap-2" @submit.prevent="send"><button type="button" @click="soon">🎙</button><input v-model="draft" class="min-w-0 flex-1 rounded-lg bg-gray-100 px-3 py-2 text-sm outline-none" placeholder="输入消息"><button type="button" @click="emoji=!emoji">😊</button><button type="button" @click="soon">🛍</button><button v-if="draft.trim()" class="rounded bg-orange-500 px-2 py-1 text-xs text-white">发送</button><button v-else type="button" @click="more=!more">＋</button></form>
...
const tools=['拍摄','相册','视频','表情','订单','商品','价保','工单进度'],emojis=['😀','😃','😄','😁','😆','😂','😊','😍','😘','😎','😭','😡','👍','👎','🙏','🎉','❤️'],camera=ref<HTMLInputElement>(),image=ref<HTMLInputElement>(),video=ref<HTMLInputElement>(),session=ref<{id:number;hash:string}|null>(null),timer=ref<number>()
