...
const request=async(action:string,options:RequestInit={})=>{const s=session.value;const url=new URL(`${apiBase}/${action}`);if(s){url.searchParams.set('id',String(s.id));url.searchParams.set('hash',s.hash)}const r=await fetch(url,{...options,headers:{'Content-Type':'application/json',...(options.headers||{})}});if(!r.ok)throw new Error('客服连接失败');return r.json()}
const load=async()=>{if(!session.value)return;const data=await request('messages');messages.value=data.items||[];status.value='店铺客服在线，您可以直接留言';await nextTick();list.value?.scrollTo({top:list.value.scrollHeight})}
const start=async()=>{try{const saved=localStorage.getItem('koshop_public_chat');if(saved)session.value=JSON.parse(saved);if(!session.value){const data=await request('start',{method:'POST',body:JSON.stringify({name:buyerName.value,avatar:auth.user?.avatar_url||'',userId:auth.user?.id||''})});session.value=data.session;localStorage.setItem('koshop_public_chat',JSON.stringify(data.session))}await load();timer.value=window.setInterval(load,3000)}catch{status.value='暂时无法连接客服，请稍后重试'}}
const show=()=>{open.value=true;void start()},close=()=>{open.value=false;more.value=false;if(timer.value)clearInterval(timer.value)},send=async()=>{const content=draft.value.trim();if(!content||!session.value)return;draft.value='';await request('send',{method:'POST',body:JSON.stringify({content})});await load()},soon=()=>alert('该功能即将上线'),goShop=()=>{close();void router.push('/')}
