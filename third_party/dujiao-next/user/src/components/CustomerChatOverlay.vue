...
const load=async()=>{if(!session.value)return;try{const data=await request('messages');messages.value=data.items||messages.value;status.value='店铺客服在线，您可以直接留言';await scroll()}catch(e:any){if(e.status===403||String(e.message).includes('会话无效')){localStorage.removeItem('koshop_public_chat');session.value=null;await start();return}status.value='消息同步失败，正在重试'}}
...
const show=()=>{open.value=true;void start()},close=()=>{open.value=false;more.value=false;if(timer.value)clearInterval(timer.value)},send=async()=>{const content=draft.value.trim();if(!content||!session.value)return;draft.value='';const tempId=`temp-${Date.now()}`;messages.value.push({id:tempId,sender:'buyer',content});await scroll();try{const data=await request('send',{method:'POST',body:JSON.stringify({content})});const i=messages.value.findIndex(x=>x.id===tempId);if(i>=0&&data.message)messages.value.splice(i,1,data.message);await load();await scroll()}catch(e:any){status.value=e.message||'消息发送失败，请重试'}},soon=()=>alert('该功能即将上线'),goShop=()=>{close();void router.push('/')}
