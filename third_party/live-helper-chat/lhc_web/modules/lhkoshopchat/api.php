...
function koshopSaveChatVariables($c, $vars) { $c->chat_variables=json_encode($vars,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES); $c->updateThis(); }
function koshopMarkSellerRead($c) { $v=koshopChatVariables($c);$v['koshop_seller_read_at']=time();$c->user_status=1;koshopSaveChatVariables($c,$v); }
function koshopMarkBuyerRead($c) { $v=koshopChatVariables($c);$v['koshop_buyer_read_at']=time();koshopSaveChatVariables($c,$v); }
...
function koshopUnreadBuyerCount($c) { $v=koshopChatVariables($c);$hasRead=array_key_exists('koshop_seller_read_at',$v);$readAt=(int)($v['koshop_seller_read_at']??0);if(!$hasRead&&(int)koshopSafeProp($c,'user_status',1)!==0)return 0;$count=0;try{foreach(erLhcoreClassModelmsg::getList(array('filter'=>array('chat_id'=>$c->id),'sort'=>'id DESC','limit'=>500)) as $m){if(koshopMessageSender($m)==='buyer'&&(int)$m->time>$readAt)$count++;}}catch(Throwable $e){}return $count; }
function koshopAttachment($raw){$x=json_decode((string)$raw,true);return is_array($x)&&isset($x['koshopType'])?$x:null;}
function koshopMessagePreview($m){if(!$m)return '暂无消息';$a=koshopAttachment($m->msg);if(!$a)return (string)$m->msg;$t=$a['koshopType']??'';if($t==='image')return '[图片]';if($t==='video')return '[视频]';if($t==='file')return '[文件]'.(!empty($a['fileName'])?' '.$a['fileName']:'');return (string)($a['text']??'附件');}
function koshopMessageReadState($m,$c){$v=koshopChatVariables($c);$at=koshopMessageSender($m)==='buyer'?(int)($v['koshop_seller_read_at']??0):(int)($v['koshop_buyer_read_at']??0);return (int)$m->time<=$at;}
function koshopMessageItem($m,$c,$sellerAvatar='') { $a=koshopAttachment($m->msg);$sender=koshopMessageSender($m);$read=koshopMessageReadState($m,$c);$sellerName=trim((string)koshopSafeProp($m,'name_support',''))?:'店铺客服';return array('id'=>(int)$m->id,'sender'=>$sender,'type'=>$a?$a['koshopType']:'text','content'=>$a?($a['text']??'附件'):(string)$m->msg,'url'=>$a?($a['url']??''):'','thumbnailUrl'=>$a?($a['thumbnailUrl']??($a['url']??'')):'','fileName'=>$a?($a['fileName']??''):'','mimeType'=>$a?($a['mimeType']??''):'','size'=>$a?(int)($a['size']??0):0,'createdAt'=>koshopBeijingDate((int)$m->time),'createdAtText'=>koshopBeijingDate((int)$m->time,'Y-m-d H:i:s'),'read'=>$read,'readText'=>$read?'已读':'未读','senderName'=>$sender==='seller'?$sellerName:koshopBuyerName($c),'senderAvatar'=>$sender==='seller'?$sellerAvatar:koshopBuyerAvatar($c),'buyerName'=>koshopBuyerName($c),'buyerAvatar'=>koshopBuyerAvatar($c),'sellerName'=>$sellerName,'sellerAvatar'=>$sellerAvatar,'card'=>$a); }
function chatItem($c) { $last=koshopLastMessage($c->id);$avatar=koshopBuyerAvatar($c);$status=(int)koshopSafeProp($c,'status',0);$unreadCount=koshopUnreadBuyerCount($c);$t=$last?(int)$last->time:(int)$c->time;return array('id'=>(int)$c->id,'buyerName'=>koshopBuyerName($c),'buyerAvatar'=>$avatar,'avatar'=>$avatar,'lastMessage'=>koshopMessagePreview($last),'lastMessageAt'=>$t?koshopBeijingDate($t,'H:i'):'','lastMessageTime'=>$t,'unread'=>$unreadCount>0,'unreadCount'=>$unreadCount,'status'=>$status===2?'closed':($status===0?'pending':'active'),'orderStatus'=>orderStatus($c->id),'isStarred'=>false,'isPinned'=>false);}

try {
    if ($action === 'unread') {
        $count = 0;
        foreach (erLhcoreClassModelChat::getList(array('sort' => 'id DESC', 'limit' => 1000)) as $unreadChat) {
    ...
    if ($action === 'conversations') {
        ...
        $params = array('sort' => 'time DESC', 'limit' => 100);
        ...
        usort($items, function($a,$b){ return $b['lastMessageTime'] <=> $a['lastMessageTime']; });
